  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        localStorage.setItem('adminName', member.name);
        navigate('/admin');
        return;
      }
    } catch {
      // Firestore unavailable — fall through to error
    }
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40 transition-shadow disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
              <><div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /><span>Checking…</span></>
            ) : (
              <><span>Login to Dashboard</span><ArrowRight size={18} /></>
            )}
          </motion.button>
        </form>
      </motion.div>
